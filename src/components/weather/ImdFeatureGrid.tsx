        ...
        {features.map((item) => {
          ...
          return (
            <button
              ...
              className="bg-white hover:bg-slate-50 border border-slate-200/90 rounded-2xl p-2 flex flex-col items-center justify-between shadow-sm hover:shadow-md transition-all text-center relative group min-h-[92px]"
            >
              {/* Badge */}
              <span className={`absolute -top-1.5 -right-1 px-1.5 py-0.2 rounded-full text-[8px] font-black tracking-tighter ${item.badgeColor} shadow-sm`}>
                {item.badge}
              </span>

              {/* Icon Container */}
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${item.color} mt-1 group-hover:scale-105 transition-transform`}>
                <Icon className="w-5 h-5" />
              </div>

              {/* Title & Sub */}
              <div className="mt-1.5 w-full">
                <span className="text-[11px] font-bold text-slate-900 leading-tight block truncate">
                  {item.title}
                </span>
